...
### Fixed
- **Login "Network error" when opening the app via its LAN address**: the frontend bundle calls the API at its absolute public URL, but `CORS_ORIGIN` only allowed the public domain — so any other origin (e.g. the LAN frontend IP) had its login request blocked by the browser and surfaced the generic network-error banner. Production `CORS_ORIGIN` now also allows the LAN frontend origin (mirrored in `backend/env.production`).
