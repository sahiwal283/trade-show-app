"""
Verbatim copy of the ORIGINAL rule-based FieldInferenceEngine (pre-improvement),
kept so the eval baseline stays reproducible under the same harness/matcher.
Run with: python3 eval/run_eval.py --engine legacy
"""
...
class LegacyFieldInferenceEngine:
    ...
    def infer_fields(self, ocr_text: str, ocr_confidence: float) -> Dict[str, Any]:
        ...
        return {
            ...
            'location': {'value': None, 'confidence': 0, 'source': 'inference'},
            'taxAmount': {'value': None, 'confidence': 0, 'source': 'inference'},
            'tipAmount': {'value': None, 'confidence': 0, 'source': 'inference'},
        }
    ...
    def _extract_merchant(self, lines: List[str], ocr_confidence: float) -> Dict[str, Any]:
        ...
        for pattern, name, confidence in known_merchants:
            if re.search(pattern, full_text):
                return {'value': name, 'confidence': confidence, 'source': 'inference'}
        ...
        rideshare_patterns = [
            ...
            (r'your trip with noel', 'Lyft', 0.85),
        ]
        ...
        for pattern in booking_patterns:
            ...
            if match:
                return {'value': match.group(1), 'confidence': 0.85, 'source': 'inference'}

        skip_words = {'thanks', 'please', 'thank', 'you', 'receipt', 'total', 'date', 'time'}
        for line in lines[:5]:
            for word in line.split():
                cleaned = re.sub(r'[^\w]', '', word)
                if (len(cleaned) > 3 and cleaned[0].isupper() and cleaned.lower() not in skip_words):
                    return {'value': cleaned, 'confidence': round(0.70 * ocr_confidence, 4), 'source': 'inference'}
    ...
    def _extract_amount(self, text: str, text_lower: str, ocr_confidence: float) -> Dict[str, Any]:
        ...
        if best_match:
            return best_match
        if weak_candidates:
            return {'value': max(weak_candidates), 'confidence': round(min(0.70 * ocr_confidence, 0.75), 4), 'source': 'inference'}
        return {'value': None, 'confidence': 0, 'source': 'inference'}
    ...
    def _extract_date(self, text: str, ocr_confidence: float) -> Dict[str, Any]:
        ...
        for pattern, pattern_conf, date_format in patterns:
            ...
            if match:
                normalized = self._normalize_date(match.group(1), date_format)
                if normalized:
                    return {'value': normalized, 'confidence': round(min(pattern_conf * ocr_confidence, 0.98), 4), 'source': 'inference'}
        return {'value': None, 'confidence': 0, 'source': 'inference'}
    ...
    def _extract_card_last_four(self, text: str, ocr_confidence: float) -> Dict[str, Any]:
        ...
        for pattern, pattern_conf in patterns:
            match = re.search(pattern, text, re.I)
            if match:
                return {'value': match.group(1), 'confidence': round(min(pattern_conf * ocr_confidence, 0.98), 4), 'source': 'inference'}
        return {'value': None, 'confidence': 0, 'source': 'inference'}
    ...
    def _predict_category(self, text_lower: str, ocr_confidence: float) -> Dict[str, Any]:
        ...
        for category, data in self.CATEGORY_KEYWORDS.items():
            ...
            weight = data['weight']
            matched = [kw for kw in keywords if self._keyword_matches_in_text(text_lower, kw)]
            if matched:
                match_score = len(matched) / len(keywords)
                confidence = min((0.5 + match_score * 0.4) * weight * ocr_confidence, 0.95)
                if confidence > best_confidence:
                    best_confidence = confidence
                    best_match = {'value': category, 'confidence': round(confidence, 4), 'source': 'inference'}
        if best_match:
            return best_match
        return {'value': None, 'confidence': 0, 'source': 'inference'}


legacy_field_inference_engine = LegacyFieldInferenceEngine()
